...
export default async function TeachersPage({
  ...
  const { items, totalPages } = await listUserAccounts("teacher", { page, search });

  return (
    <div className="flex flex-col gap-4">
      <PortalBreadcrumb
        items={[{ label: t("dashboard"), href: "/admin" }, { label: t("teachers") }]}
      />
      <h1 className="text-xl font-semibold">{t("teachers")}</h1>

      <ListSearch className="sm:max-w-sm" />

      <UserAccountsTable role="teacher" items={items} />

      <ListPagination page={page} totalPages={totalPages} searchParams={params} />

      <UserAccountCreateButton role="teacher" />
    </div>
  );
}
